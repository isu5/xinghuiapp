...
class My extends TagLib {
    ...
    protected $tags=array(
        ...
        'webuploaderfile'=>array('attr'=>'name,url,word','close'=>0),
        'webuploaderavatar'=>array('attr'=>'name,url,word','close'=>0),
...
php;
     ...
     * word：提示文字
     */
    public function _webuploaderfile($tag){
        ...
        $id_name='upload-'.md5(rand(1000,9999));
            $str=<<<php
...
jQuery(function() {
    ...
    uploader = WebUploader.create({
        ...
        accept: {
            title: 'intoTypes',
            extensions: 'doc,xls,docx,xlsx,pdf,txt',
            mimeTypes: '.doc,.xls,.docx,.xlsx,.pdf,.txt'
         },
...
/**
     ...
     * word：提示文字
     */
    public function _webuploaderavatar($tag){
        $url=isset($tag['url'])?$tag['url']:U('Home/Index/ajax_upload');
        $name=isset($tag['name'])?$tag['name']:'file_name';
        $word=isset($tag['word'])?$tag['word']:'或将照片拖到这里，单次最多可选300张';
        $id_name='upload-'.uniqid();
